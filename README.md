...
| 스킬 | 역할 |
|---|---|
| `backtest-report` | 백테스트 산출물(`equity.csv`/`trades.csv`/`summary.json`)과 전략 명세를 묶어 자립형 HTML 리포트 한 장으로 만든다. 수익률 지표·자본곡선·거래 규칙·거래 저널이 고정된 디자인 규약으로 들어가서, 실행 회차가 달라도 리포트 형식은 그대로 유지된다. |
